...
#   geom_bar(stat="identity") +
#   geom_point(aes(x=region,y=MeanNomRate)) + 
...
```{r}
genUsersData <- itkonen[(itkonen$workerid %in% genUsers$workerid),]
```
...
itkonen %>% 
  ...
  filter(polarity=="positive") %>% 
  group_by(genUser,responseCase) %>% 
  count() %>% 
  pivot_wider(values_from = n, names_from = responseCase) %>% 
  summarize(genRate = gen/(nom+gen)) %>% 
  ggplot(aes(x=genUser,y=genRate)) + 
  geom_bar(stat="identity")
```
...
- Nominative under negation is more common with passives than with actives, and there is a correlation such that participants who use more nominative under negated passives also use more nominative under negated actives
- Participants who use genitive under negation are slightly more likely to use genitive in positive polarity constructions than those who categorically avoid genitive under negation
- 

```{r}
itkonenTable <- itkonen %>% 
  filter(attention == "critical") %>% 
  filter(item != "jokin2")
```

```{r}
table(itkonenTable$responseCase,itkonenTable$polarity,itkonenTable$voice)
```
